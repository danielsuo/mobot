...
addParamValue(p, 'ConnectPoints', true, @(x) validateattributes(x, ...
    {'logical'}, {'scalar'}));
addParamValue(p, 'Anchored', false, @(x) validateattributes(x, ...
    {'logical'}, {'scalar'}));
addParamValue(p, 'AnchorX', 0, @(x) validateattributes(x, ...
    {'numeric'}, {'scalar'}));
addParamValue(p, 'AnchorY', 0, @(x) validateattributes(x, ...
    {'numeric'}, {'scalar'}));
...
% Anchors to draw line
anchor_x = p.Results.AnchorX;
anchor_y = p.Results.AnchorY;
anchored = p.Results.Anchored;
...
%--------------------------------------------------------------------------
    function mouseMoveFcn(varargin)
        ...
        if anchored
            if exist('previewLine')
               if ishandle(previewLine)
                   delete(previewLine);
               end
            end
            ...
            curr_x = pt(1);
            curr_y = pt(3);
            previewLine = line([anchor_x curr_x], [anchor_y curr_y], 'Color', 'g');
        end
