function data = loadSUN3D(sequenceName, frameIDs, SUN3Dpath)
    ...
    if isempty(imageFiles)
        imageFiles = dirSmart(fullfile(SUN3Dpath,sequenceName,'image/'),'tif');
    end
    imgaefolder = 'image/';
    if isempty(imageFiles)
        imageFiles = dirSmart(fullfile(SUN3Dpath,sequenceName,'color/'),'tif');
        imgaefolder = 'color/';
    end
    ...
    for i=1:length(imageFiles)
        id_time = sscanf(imageFiles(i).name(1:end-4), '%ld-%ld');
    ...
    for i=1:length(depthFiles)
        id_time = sscanf(depthFiles(i).name(1:end-4), '%ld-%ld');
    ...
    for frameID=frameIDs
        cnt = cnt + 1;        
        if isempty(jpgFiles)
            data.image{cnt} = fullfile(fullfile(SUN3Dpath,sequenceName,imgaefolder,imageFiles(frameID).name));
        else
            data.image{cnt} = fullfile(fullfile(SUN3Dpath,sequenceName,imgaefolder,imageFiles(frameID).name));
        end
    ...
    data.depthTimestamp = depthTimestamp(IDimage2depth);
