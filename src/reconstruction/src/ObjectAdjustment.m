...
%{
TYPE 0 = floor
TYPE 1 = vertical axis align
TYPE 2 = 
TYPE 3 = mirror
TYPE 4 = ceiling. no rotation. only height
...
for label=1:objectLabel.length
    if ~isempty(data.annotation.objects{label})
        ...
        beginPos = strfind(data.annotation.objects{label}.name,'[');
        endPos = strfind(data.annotation.objects{label}.name,']');
        if ~isempty(beginPos) && ~isempty(endPos)
            str = data.annotation.objects{label}.name(beginPos+1:endPos-1);
            str = sscanf(str,'%f,%f,%f,%f');
            if length(str)==4
                userSpecified = true;
                objectLabel.objectSize(label,:) = str(1:3); % unknown object = no constrains;
                objectLabel.objectType(label)   = str(4);
                fprintf('object with user specified size (%d): %s = [%f %f %f %d]\n',label,category, str(1), str(2), str(3), str(4));                
            end
        end
...
for objectID=1:objectLabel.length
    if ~isempty(data.annotation.objects{objectID})
        pos = strfind(data.annotation.objects{objectID}.name,'w(');
        if ~isempty(pos)
            str = data.annotation.objects{objectID}.name(pos(1)+2:end);
            pos = strfind(str,')');
            if ~isempty(pos)
                str = str(1:pos(1)-1);        
                newW = str2double(str);

                fprintf('Specified Weight: Object %d  CNT=%d  TYPE=%d  WEIGHT=%f=>%f  Size %.2fx%.2fx%.2f: %s\n',...
                    objectID,cntPoints(objectID),objectLabel.objectType(objectID),objectLabel.optimizationWeight(objectID),newW,...
                ...
                objectLabel.optimizationWeight(objectID) = newW;
            end
        end
    end
end
